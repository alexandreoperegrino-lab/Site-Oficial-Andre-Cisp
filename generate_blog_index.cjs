...
const articles = [
  ...
  },
  {
    ...
    desc: 'Conheça os equipamentos que farão diferença na sua bancada: desde as chaves certas até fontes de bancada e microscópio.'
  },
  {
  ...
  },
  {
    ...
    desc: 'Aprenda a analisar oscilações por USB tester e se certifique da substituição ideal ou do reparo pontual de trilhas.'
  },
  {
  ...
  },
  {
    ...
    desc: 'Troca periódica de pasta térmica a partir de dissipadores velhos e obstrutivos previne o colapso e o afogamento térmico.'
  },
  {
  ...
  },
  {
    ...
    desc: 'Tire a prova definitiva de que se educar através das tele-aulas eliminou o aprendizado defasado do reparo técnico de placas.'
  },
  {
  ...
  },
  {
    ...
    desc: 'Porque ser parceiro do aquecimento em soldas para chipsets gráficos pode explodir seu faturamento num nível absurdo.'
  },
  {
    ...
    desc: 'Conquiste clientes de maneira fiel: devolução de peças, transparência total, higienização rigorosa e atendimento inesquecível.'
  }
];

let itemsHtml = articles.map(a => `
  <a href="/blog/${a.slug}.html" class="card-tech rounded-2xl overflow-hidden flex flex-col group relative bg-navy-800 hover:shadow-[0_0_20px_rgba(249,115,22,0.15)] transition-all">
      <div class="h-48 overflow-hidden relative">
          <div class="absolute inset-0 bg-gradient-to-b from-transparent to-navy/70 z-10 pointer-events-none"></div>
          <img src="${a.image}" alt="${a.title}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
      </div>
      <div class="p-6">
          <h4 class="text-xl font-bold font-display text-white mb-3 group-hover:text-tech-orangeGlow transition-colors">${a.title}</h4>
          <p class="text-sm text-tech-light/70">${a.desc}</p>
      </div>
  </a>
`).join('');
...
<head>
    ...
    <meta name="description" content="Manutenção e Consertos Avançados - Blog Profissional">
    <title>Blog - Dicas de Eletrônica e Conserto de Celular | André Cisp</title>
...
<body class="bg-navy text-tech-light font-sans antialiased overflow-x-hidden selection:bg-tech-orange/30 selection:text-white">
    ...
    <section class="pt-32 pb-24 md:pt-40 bg-navy-900 min-h-screen">
        <div class="container mx-auto px-6 max-w-6xl relative z-10">
            <div class="text-center mb-16">
                <div class="inline-flex items-center gap-2 mb-3">
                    <span class="h-px w-8 bg-tech-orangeGlow"></span>
                    <h2 class="text-sm font-bold text-tech-orangeGlow tracking-widest uppercase">Conteúdo Profissional</h2>
                    <span class="h-px w-8 bg-tech-orangeGlow"></span>
                </div>
                <h3 class="text-3xl md:text-5xl font-display font-bold text-white mb-6">Dicas de <span class="text-tech-orangeGlow">Eletrônica</span></h3>
                <p class="text-tech-light/80 max-w-2xl mx-auto text-lg">Acesse nossos artigos gratuitos para evoluir suas habilidades de manutenção em bancada e turbinar seus lucros.</p>
            </div>
...
fs.writeFileSync(path.join('blog', 'index.html'), listHtml);
